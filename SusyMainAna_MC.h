 ...
 */
	...
	// $Id: SusyMainAna_MC.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
	//

#ifndef SusyMainAna_MC_h
#define SusyMainAna_MC_h
...
class SusyMainAna_MC {
	...
	SusyMainAna_MC(TTree *tree=0);
	virtual ~SusyMainAna_MC();
...
#ifdef SusyMainAna_MC_cxx
SusyMainAna_MC::SusyMainAna_MC(TTree *tree)
{
	if (tree == 0) {
...
SusyMainAna_MC::~SusyMainAna_MC()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyMainAna_MC::GetEntry(Long64_t entry)
{
		// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyMainAna_MC::LoadTree(Long64_t entry)
{
...
void SusyMainAna_MC::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyMainAna_MC::Initialize() {
...
void SusyMainAna_MC::IncludeAJson(std::string jsonfile) {
...
bool SusyMainAna_MC::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyMainAna_MC_cxx
