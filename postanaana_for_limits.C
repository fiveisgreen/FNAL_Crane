#define postanaana_for_limits_C
...
void postanaana_for_limits(){
        int nKinemVars = nKinemVars_all;
        string *s_KinemVars = s_KinemVars_all;

	int debug = 0;
	if(debug>0) cout<<"postanaana_for_limits is alive!"<<endl;
	CMSStyle();
	...
	cout<<endl<<"Writing To File "<<plotsAndBackground_data_lim <<endl<<endl;
	TFile* fout = new TFile(plotsAndBackground_data_lim.c_str(),"RECREATE");
	ofstream logfile;
	logfile.open(logplotsAndBackground_data_lim.c_str());

	ofstream integSyst_file;
	integSyst_file.open(integSyst_data_lim.c_str());
...
//	TGraphAsymmErrors* g[nEventTopologies];
	...
	for (int iTopo=0; iTopo<nEventTopologies; iTopo++) {
		for (int kKinVar = 0; kKinVar<nKinemVars; kKinVar++) {
			find_bkg_with_fit(lha2[s_EventTopology[iTopo]][s_KinemVars[kKinVar]],B_integral[iTopo], 0, lsb_int->GetBinContent(iTopo), usb_int->GetBinContent(iTopo));//if the side bands have a 0, this'll spew warnings, but it's usually ok. 
		}
	}
...
//hereherehere
	...
	for (int iTopo=0; iTopo<nEventTopologies; iTopo++){
		...
		float integ_syst = 1.0+ (B_integral_error[iTopo]/B_integral[iTopo]);
		if(TMath::IsNaN(integ_syst)) integ_syst = 1.0;
		integSyst_file<<Form("%s %f\n",(char*)s_EventTopology[iTopo].c_str(),integ_syst);
	}
	logfile.close();
	integSyst_file.close();
	fout->cd();
	fout->Write();
	fout->Close();
}//end postanaana_for_limits
