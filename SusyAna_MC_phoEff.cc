...
//fill Loose
		...
		//use this one!
		bool SusyAna_MC_phoEff::ok_muon_DMoris(std::vector<susy::Muon>::iterator it_Mu,susy::Track& innerTrack){
		...
		//use this one!
		bool SusyAna_MC_phoEff::ok_ele(std::vector<susy::Electron>::iterator it_Ele){	//Dave Moris
		...
		bool SusyAna_MC_phoEff::ok_ele_AN_11_409(std::vector<susy::Electron>::iterator it_Ele, susy::Track& innerTrack, float pVtx_Z){
			//if(!it_Ele->isPF() and !it_Ele->passingMvaPreselection()) return false;
			float pt=it_Ele->momentum.Pt();
...
float SusyAna_MC_phoEff::JEC_MET(susy::MET* pfmet,std::vector<susy::PFJet*> ra3_pfjets,float &corrX, float &corrY, bool Debug_noisily){
...
float SusyAna_MC_phoEff::PhoEC_MET(float oldcorrmet,std::vector<susy::Photon*> photons, float &corrX, float &corrY, bool Debug_noisily){
		...
		bool SusyAna_MC_phoEff::ok_vtx(std::vector<susy::Vertex>::iterator it){
