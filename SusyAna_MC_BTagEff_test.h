...
// Class:      SusyAna_MC_BTagEff_test.h
// 
/*
...
// $Id: SusyAna_MC_BTagEff_test.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
//

#ifndef SusyAna_MC_BTagEff_test_h
#define SusyAna_MC_BTagEff_test_h
...
class SusyAna_MC_BTagEff_test {
	public :
		...
		SusyAna_MC_BTagEff_test(TTree *tree=0);
		virtual ~SusyAna_MC_BTagEff_test();
...
#ifdef SusyAna_MC_BTagEff_test_cxx
SusyAna_MC_BTagEff_test::SusyAna_MC_BTagEff_test(TTree *tree)
{
	if (tree == 0) {
...
SusyAna_MC_BTagEff_test::~SusyAna_MC_BTagEff_test()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyAna_MC_BTagEff_test::GetEntry(Long64_t entry)
{
	// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyAna_MC_BTagEff_test::LoadTree(Long64_t entry)
{
...
void SusyAna_MC_BTagEff_test::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyAna_MC_BTagEff_test::Initialize() {
...
void SusyAna_MC_BTagEff_test::IncludeAJson(std::string jsonfile) {
	...
	bool SusyAna_MC_BTagEff_test::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyAna_MC_BTagEff_test_cxx
