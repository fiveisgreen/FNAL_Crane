...
void SusyMainAna_MC::InitializePerEvent() {
	
}


bool SusyMainAna_MC::isSameObject(TLorentzVector& p1, TLorentzVector& p2) {
...
float SusyMainAna_MC::d0correction(TVector3& beamSpot, susy::Track& track) const {
	return track.d0() - beamSpot.X()*std::sin(track.phi()) + beamSpot.Y()*std::cos(track.phi());
}


bool SusyMainAna_MC::PassTrigger(TString path) {
...
bool SusyMainAna_MC::PassTriggers() {
	bool pass = false;
...
///////////////////////////////////////////////////////
	...
	int nStop = 0;
	int nNotStop = 0;
	TRandom *rand_smearing = new TRandom();//make TRandom for use in photon smearing. Keep it here so seed doesn't repeate. 
	
	if(pl>=3) printf("Enter Main Analysis Loop\n");

		// start event looping
	Long64_t nbytes = 0, nb = 0;
	if(pl>=9) printf("n events to process: %i\n",processNEvents);
...
//    
		...
		if(phoMap != event->photons.end()) {
			for(std::vector<susy::Photon>::iterator it = phoMap->second.begin();
				...
				//smear all photons. 
				it->MVAregEnergyAndErr.first *= MCsmear(it->caloPosition.Eta(), it->r9 , rand_smearing );
...
//                                 
		...
		//susy::MuonCollection muonC = event->muons["muidGlobalMuonPromptTight"];
			//muonCollectionTags
...
//                                   
		...
		//in retrospect, I don't know what I'm complaining about here..
...
/*		bool Tpho =
		...
		std::vector<int> bTagRequirement_TM;
		...
		bTagRequirement_TM.push_back(2);
		...
		bTagRequirement_T.push_back(1);
		bTagRequirement_TL.push_back(1);
		bTagRequirement_TM.push_back(1);
		bTagRequirement_TT.push_back(2);
...
//			dijetPt01 > 20;
		...
		//if(pl > 0) std::cout << "3boo5" << std::endl;
...
//		topoCut["4JbTLLLm20"] =  nJ >=4 && nbL >=4 && nbT >=1 && corrmet >20.0;
		...
		topoCut["1Elegbar2"] = pfEles.size() >=1 && twoPhoBar;//f
...
//		topoCut["2JbMLgbar2Mpho"] = Mpho && topoCut["2JbMLgbar2"];
		...
		topoCut["23JbML!gbar2Mbb0lep"] = nJ>=2 && nJ <=3 && nbL ==2 && nbM >=1 && twoPhoBar && nLep == 0 && Mbb01_is_H;
                m_BTagWeight["23JbML!gbar2Mbb0lep"] = new BTagWeight(3,2,3,bTagRequirement_ML,bTypeExactLoose);
		topoCut["2JbML!gbar2Mbb0lep"]  = nJ==2           && nbL ==2 && nbM >=1 && twoPhoBar && nLep == 0 && Mbb01_is_H;
                m_BTagWeight["2JbML!gbar2Mbb0lep"] = new BTagWeight(3,2,bTagRequirement_ML,bTypeExactLoose);
		topoCut["23JbML!gbar2Mbb01lep"] = nJ>=2 && nJ <=3 && nbL ==2 && nbM >=1 && twoPhoBar && nLep < 2 && Mbb01_is_H;                                                                                                                                            
                m_BTagWeight["23JbML!gbar2Mbb01lep"] = new BTagWeight(3,2,3,bTagRequirement_ML,bTypeExactLoose);
		topoCut["2JbML!gbar2Mbb01lep"]  = nJ==2           && nbL ==2 && nbM >=1 && twoPhoBar && nLep < 2 && Mbb01_is_H; 
		...
		topoCut["2JbT!gbar2ProbeMJJ"]=nJ>=2 && nbT >= 1 && twoPhoBar && ProbeMJJ && nbL < 2 && nLep ==0; 
		        m_BTagWeight["2JbT!gbar2ProbeMJJ"] = new BTagWeight(3,2,2,99,bTagRequirement_T,bTypeExactTight);//at most 1 b-jet

		topoCut["23Jb01MewkMll0lepgbar2"]= nJ>=2 && nJ <=3 && MllEWK && nLep == 0 && twoPhoBar && nbL <=1 && nbT<=0; //for WH and ZH
		        m_BTagWeight["23Jb01MewkMll0lepgbar2"] = new BTagWeight(3,2,3,2,99, bTagRequirement_M, b_veto);//at most 1 M b-jet

		topoCut["0lep25Jb01MewkMllgbar2"] = nJ>=2 && nJ <=5 && MllEWK && nLep == 0 && twoPhoBar && nLep == 0 && nbL <=1 && nbT<=0;
			m_BTagWeight["0lep25Jb01MewkMllgbar2"] = new BTagWeight(3,2,5,2,99, bTagRequirement_M, b_veto);//xxxy

		topoCut["1!lep23Jb01MewkMllgbar2"] = nJ>=2 && nJ <=3 && MllEWK && nLep == 0 && twoPhoBar && nLep == 1 && nbL <=1 && nbT<=0;
			m_BTagWeight["1!lep23Jb01MewkMllgbar2"] = new BTagWeight(3,2,3,2,99, bTagRequirement_M, b_veto);//xxxy

		topoCut["1!lep23Jb01M!ewkMllgbar2"]= nJ>=2 && nJ <=3 && (!MllEWK) && nLep == 0 && twoPhoBar && nLep == 1 && nbL <=1 && nbT<=0; //for WWbins and ZZbins
		...
		topoCut["2JbML!gbar2bestOn0lep"]=nJ>=2 && nbL ==2 && nbM >=1 && twoPhoBar && nLep ==0 && bestMjj_is_H;
                m_BTagWeight["2JbML!gbar2bestOn0lep"] = new BTagWeight(3,2,bTagRequirement_ML,bTypeExactLoose);
		topoCut["4JbML!gbar2ewkMllbestOff0lep"]=nJ>=4 && nbL ==2 && nbM >=1 && MllEWK && twoPhoBar && (!bestMjj_is_H) && nLep==0;
		        m_BTagWeight["4JbML!gbar2ewkMllbestOff0lep"] = new BTagWeight(3,4,99,2,99,bTagRequirement_ML, bTypeExactLoose);
		topoCut["2JbML!gbar2bothOff0lep"]=nJ>=2 && nbL ==2 && nbM >=1 && twoPhoBar && nLep ==0 && (!bestMjj_is_H) && !topoCut["4JbML!gbar2ewkMllbestOff0lep"];
                m_BTagWeight["2JbML!gbar2bothOff0lep"] = new BTagWeight(3,2,bTagRequirement_ML,bTypeExactLoose);
		topoCut["01J0lep0Bgbar2"] = nJ <=1 && nbL ==0 && nLep == 0 && twoPhoBar;
		...
		for (int iTopo=0; iTopo<nEventTopologies; iTopo++) {
			if(topoCut[s_EventTopology[iTopo]]){
				...
				if(pbFill && pl > 0) std::cout << "topo: "<<s_EventTopology[iTopo] << std::endl;
...
//					lh_unsliced[s_EventTopology[iTopo]]["dijetDPhi01"]->Fill(dPhi(j0,j1),weight);//
				...
				if(pbFill && pl >=4) std::cout << "boo6" << std::endl;
...
//	}//end if filter enabled
	...
	if(pbFinisUp && pl >=5) cout<<"did eventlist"<<endl;
...
bool SusyMainAna_MC::ok_muon(std::vector<susy::Muon>::iterator it){ //Yu Feng
...
bool SusyMainAna_MC::ok_muon_AN_11_409(std::vector<susy::Muon>::iterator it,susy::Track& combinedTrack, float pVtx_Z){ //probably inner track instead
...
//	std::map<TString,UChar_t>::iterator idPair = it->idPairs.find("muidGlobalMuonPromptTight");
	...
	//use this one!
bool SusyMainAna_MC::ok_muon_DMoris(std::vector<susy::Muon>::iterator it_Mu,susy::Track& innerTrack){
	...
	//use this one!
bool SusyMainAna_MC::ok_ele(std::vector<susy::Electron>::iterator it_Ele){	//Dave Moris
...
bool SusyMainAna_MC::ok_ele_AN_11_409(std::vector<susy::Electron>::iterator it_Ele, susy::Track& innerTrack, float pVtx_Z){
		//if(!it_Ele->isPF() and !it_Ele->passingMvaPreselection()) return false;
	float pt=it_Ele->momentum.Pt();
...
float SusyMainAna_MC::JEC_MET(susy::MET* pfmet,std::vector<susy::PFJet*> ra3_pfjets,float &corrX, float &corrY, bool Debug_noisily){ 
...
float SusyMainAna_MC::PhoEC_MET(float oldcorrmet,std::vector<susy::Photon*> photons, float &corrX, float &corrY, bool Debug_noisily){
...
bool SusyMainAna_MC::ok_vtx(std::vector<susy::Vertex>::iterator it){
...
void SusyMainAna_MC::MakeHistSet(TH1F** h, string prefix, int nbins, float lower, float upper){
...
void SusyMainAna_MC::MakeHistSet(TH1F** h, string prefix, int nbins, float*  bins){
	string names[nPhoMassAndBkgDists];
	string titles[nPhoMassAndBkgDists];
	string descriptions[nPhoMassAndBkgDists] = {"Lower Side Band",
		"in Higgs Tag Region","Upper Side Band","Background to Higgs Tag Region",
