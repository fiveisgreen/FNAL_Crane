...
*/		
	...
	for (Long64_t jentry=0; jentry < processNEvents; jentry++) {
		...
		if (vloose_photons.size()<1 || electronish_photons.size()<1 ) continue; 
		nCnt[1]++;//have 2 photons

			// sort photons by Et
		std::sort(vloose_photons.begin(),vloose_photons.end(),EtGreater<susy::Photon>);
		std::sort(electronish_photons.begin(),electronish_photons.end(),EtGreater<susy::Photon>);
		if(vloose_photons[0]->momentum.Et()<(phoEtThresh0-5) && electronish_photons[0]->momentum.Et()<(phoEtThresh0-5) ) continue;

		bool ok_photons = false;
		//std::sort(vloose_photons.begin(),vloose_photons.end(),EtGreater<susy::Photon>);
		//keep the event if there are two vloose photons, with Pt over 36, 22 threshold and 80<mgg
	    	for(std::vector<susy::Photon*>::iterator i_p1 = vloose_photons.begin(); i_p1 != vloose_photons.end() && !ok_photons; i_p1++) {
			if((*i_p1)->momentum.Et()<phoEtThresh0-5){ //if this isn't over thresh
				for(std::vector<susy::Photon*>::iterator i_p2 = electronish_photons.begin(); i_p2 != electronish_photons.end() && !ok_photons; i_p2++) {
					if((*i_p2)->momentum.Et()<phoEtThresh1-5) break;//neither is over thresh
					if( ((*i_p1)->momentum + (*i_p2)->momentum).M() > 80.0){
						ok_photons = true;
						nCnt[2]++;
					}//end if
				}//end if for each ele. 
			}
			else{ //else pho is over lead thresh
				for(std::vector<susy::Photon*>::iterator i_p2 = electronish_photons.begin(); i_p2 != electronish_photons.end() && !ok_photons; i_p2++) {
					if( ((*i_p1)->momentum + (*i_p2)->momentum).M() > 80.0){
						ok_photons = true;
						nCnt[2]++;
