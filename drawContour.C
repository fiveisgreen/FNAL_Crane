...
// 	      << exp_m1s << ", " << exp_m2s << ", " << exp_p1s << ", " << exp_p2s << std::endl;
  ...
  //  can_diff->Divide(nlimit,nxs);
	TH2D* h_excl[nlimit][nxs-1]; //2d array of histograms, each a clone of the ith limit hist
  for(int i=0; i<nlimit; i++) {
    for(int j=0; j<nxs-1; j++) {
      ...
      int nbinsy = h_excl[i][j]->GetYaxis()->GetNbins();
		...
		for( int ibx=1; ibx<=nbinsx; ++ibx){ //for every bin
			for( int iby=1; iby<=nbinsy; ++iby){
				double x1 = h_limit[i]->GetBinContent(ibx,iby);//x1 and x2 have nothing to do with x-values.
				double x2 = h_xs[j]->GetBinContent(ibx,iby);
					//fill the h_excl with limit - xs. //This only makes sense if the limit is expressed as a cs.
	...
	for(int i=0; i<nlimit; i++) {
		for(int j=0; j<nxs-1; j++) {
			...
			if(useCustomGetContour) {
				curv[i][j] = getContour(h_excl[i][j],"excl_curv_"+limitname[i]+"_"+xsname[j]);//Get Contour! see util
				curv[i][j]->Draw("SAME L");
			}
			else {
				h_excl[i][j]->SetContour(2,contours); //set to draw one of two level. 0 or 1
				h_excl[i][j]->Draw("SAME CONT LIST"); //draw same, as contour. //also set the contour level.
				gPad->Update();
