 ...
 */
	...
	// $Id: SusyAna_smearing.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
	//

#ifndef SusyAna_smearing_h
#define SusyAna_smearing_h
...
class SusyAna_smearing {
	...
	SusyAna_smearing(TTree *tree=0);
	virtual ~SusyAna_smearing();
...
#ifdef SusyAna_smearing_cxx
SusyAna_smearing::SusyAna_smearing(TTree *tree)
{
	if (tree == 0) {
...
SusyAna_smearing::~SusyAna_smearing()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyAna_smearing::GetEntry(Long64_t entry)
{
		// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyAna_smearing::LoadTree(Long64_t entry)
{
...
void SusyAna_smearing::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyAna_smearing::Initialize() {
...
void SusyAna_smearing::IncludeAJson(std::string jsonfile) {
...
bool SusyAna_smearing::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyAna_smearing_cxx
