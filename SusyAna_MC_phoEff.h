...
// $Id: SusyAna_MC_phoEff.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
//

#ifndef SusyAna_MC_phoEff_h
#define SusyAna_MC_phoEff_h
...
class SusyAna_MC_phoEff {
	public :
		...
		SusyAna_MC_phoEff(TTree *tree=0);
		virtual ~SusyAna_MC_phoEff();
...
#ifdef SusyAna_MC_phoEff_cxx
SusyAna_MC_phoEff::SusyAna_MC_phoEff(TTree *tree)
{
	if (tree == 0) {
...
SusyAna_MC_phoEff::~SusyAna_MC_phoEff()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyAna_MC_phoEff::GetEntry(Long64_t entry)
{
	// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyAna_MC_phoEff::LoadTree(Long64_t entry)
{
...
void SusyAna_MC_phoEff::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyAna_MC_phoEff::Initialize() {
...
void SusyAna_MC_phoEff::IncludeAJson(std::string jsonfile) {
	...
	bool SusyAna_MC_phoEff::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyAna_MC_phoEff_cxx
