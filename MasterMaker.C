...
void MasterMaker(){
	...
	jdlfile.open("StopGrid_lowmass.jdl");
	string executable = "run.sh";
	...
	runfile<<"echo $point_for_b"<<endl;
	runfile<<"echo $file_for_b"<<endl;
	runfile<<"set point_for_b=`echo ${1} | sed 's/wwaa/bbaa/;s/ttaa/bbaa/;s/zzaa/bbaa/'`"<<endl;
	runfile<<"set file_for_b=`echo ${2} | sed 's/wwaa/bbaa/;s/ttaa/bbaa/;s/zzaa/bbaa/;s/_5_hh/_15_hh/;s/_10_hh/_15_hh/;s/_13_hh/_15_hh/'`"<<endl;

	//runfile<<"root -l -b -q ana_MC_BTagEff_arg.C\\(\\\"${1}\\\",\\\"${2}\\\"\\)"<<endl;
	runfile<<"root -l -b -q ana_MC_BTagEff_arg.C\\(\\\"$point_for_b\\\",\\\"$file_for_b\\\"\\)"<<endl;
	runfile<<"root -l -b -q ana_MC_BTagEff_arg.C\\(\\\"$point_for_b\\\",\\\"$file_for_b\\\"\\)"<<endl;
	runfile<<""<<endl;
	...
	runfile<<"date"<<endl;
        runfile<<"cat doLimits_${1}.sh | grep -v ! > doLimits_${1}_mod.sh"<<endl;
        runfile<<"source doLimits_${1}_mod.sh"<<endl;
	runfile<<"date"<<endl;
	runfile<<"./simplifyLimitBundle.py LimitResultBundle_${1}.txt"<<endl; //make simplified result file
	runfile<<"echo \"Load Up result\""<<endl;
        runfile<<"tar -cvf Cards_${1}.tar LimitCard*.txt"<<endl;
        runfile<<"tar -uvf ${1}.tar LimitResultBundle_${1}.txt LimitResultBundle_${1}_reduced.txt Cards_${1}.tar"<<endl;
	runfile<<"cp ${1}.tar ../../"<<endl;
	runfile<<"cd ../../"<<endl;
	runfile<<"pwd"<<endl;
