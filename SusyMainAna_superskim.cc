	...
	// -*- C++ -*-
#define SusyMainAna_superskim_cxx
...
#include "SusyMainAna_superskim.h"
...
void SusyMainAna_superskim::InitializePerEvent() {
	
}


bool SusyMainAna_superskim::isSameObject(TLorentzVector& p1, TLorentzVector& p2) {
...
float SusyMainAna_superskim::d0correction(TVector3& beamSpot, susy::Track& track) const {
	
	return track.d0() - beamSpot.X()*std::sin(track.phi()) + beamSpot.Y()*std::cos(track.phi());
}


bool SusyMainAna_superskim::PassTrigger(TString path) {
...
bool SusyMainAna_superskim::PassTriggers() {
	bool pass = false;
...
void SusyMainAna_superskim::Loop() {
	printLevel = 0;
...
/*	nbins["EtaGG"] = 26;
	...
	//const int Nr9Triggers = 4;
		...
		//bool passHLTs[NT];
	TString dmason_triggers[5] = {  //not using these. 
	...
	//for (Long64_t jentry=0; jentry < 1000; jentry++){  //master loop
...
///big heap of machienry ripped out here
...
bool SusyMainAna_superskim::ok_muon(std::vector<susy::Muon>::iterator it){ //Yu Feng
...
bool SusyMainAna_superskim::ok_muon_AN_11_409(std::vector<susy::Muon>::iterator it,susy::Track& combinedTrack, float pVtx_Z){ //probably inner track instead
...
//	std::map<TString,UChar_t>::iterator idPair = it->idPairs.find("muidGlobalMuonPromptTight");
	...
	//use this one!
bool SusyMainAna_superskim::ok_muon_DMoris(std::vector<susy::Muon>::iterator it_Mu,susy::Track& innerTrack){
	...
	//use this one!
bool SusyMainAna_superskim::ok_ele(std::vector<susy::Electron>::iterator it_Ele){	//Dave Moris
...
bool SusyMainAna_superskim::ok_ele_AN_11_409(std::vector<susy::Electron>::iterator it_Ele, susy::Track& innerTrack, float pVtx_Z){
		//if(!it_Ele->isPF() and !it_Ele->passingMvaPreselection()) return false;
	float pt=it_Ele->momentum.Pt();
...
float SusyMainAna_superskim::JEC_MET(susy::MET* pfmet,std::vector<susy::PFJet*> ra3_pfjets,float &corrX, float &corrY, bool Debug_noisily){ 
...
float SusyMainAna_superskim::PhoEC_MET(float oldcorrmet,std::vector<susy::Photon*> photons, float &corrX, float &corrY, bool Debug_noisily){
...
bool SusyMainAna_superskim::ok_vtx(std::vector<susy::Vertex>::iterator it){
...
void SusyMainAna_superskim::MakeHistSet(TH1F** h, string prefix, int nbins, float lower, float upper){
