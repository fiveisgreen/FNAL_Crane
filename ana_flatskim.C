...
void ana_flatskim(TString ds="relval", TString physics="ttbar") {
  ...
  // Main analysis code
  gROOT->LoadMacro("SusyFlatSkim.cc+");

  // chain of inputs
  TChain* chain = new TChain("susyTree");
  chain->Add("SusyEvents_skim_2lep_heavy.root");
  ...
  SusyFlatSkim* sea = new SusyFlatSkim(chain);
