	...
	// Class:      SusyEventAnalyzer_shruti.h
	// 
/*
 ...
 */
	...
	// $Id: SusyEventAnalyzer_shruti.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
	//

#ifndef SusyEventAnalyzer_shruti_h
#define SusyEventAnalyzer_shruti_h
...
class SusyEventAnalyzer_shruti {
	public :
	...
	SusyEventAnalyzer_shruti(TTree *tree=0);
	virtual ~SusyEventAnalyzer_shruti();
...
#ifdef SusyEventAnalyzer_shruti_cxx
SusyEventAnalyzer_shruti::SusyEventAnalyzer_shruti(TTree *tree)
{
	if (tree == 0) {
...
SusyEventAnalyzer_shruti::~SusyEventAnalyzer_shruti()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyEventAnalyzer_shruti::GetEntry(Long64_t entry)
{
		// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyEventAnalyzer_shruti::LoadTree(Long64_t entry)
{
...
void SusyEventAnalyzer_shruti::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyEventAnalyzer_shruti::Initialize() {
...
void SusyEventAnalyzer_shruti::IncludeAJson(std::string jsonfile) {
...
bool SusyEventAnalyzer_shruti::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyEventAnalyzer_shruti_cxx
