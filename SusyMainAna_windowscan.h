	...
	// Class:      SusyMainAna_windowscan.h
	// 
/*
 ...
 */
	...
	// $Id: SusyMainAna_windowscan.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
	//

#ifndef SusyMainAna_windowscan_h
#define SusyMainAna_windowscan_h
...
class SusyMainAna_windowscan {
	...
	SusyMainAna_windowscan(TTree *tree=0);
	virtual ~SusyMainAna_windowscan();
...
#ifdef SusyMainAna_windowscan_cxx
SusyMainAna_windowscan::SusyMainAna_windowscan(TTree *tree)
{
	if (tree == 0) {
...
SusyMainAna_windowscan::~SusyMainAna_windowscan()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyMainAna_windowscan::GetEntry(Long64_t entry)
{
		// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyMainAna_windowscan::LoadTree(Long64_t entry)
{
...
void SusyMainAna_windowscan::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyMainAna_windowscan::Initialize() {
...
void SusyMainAna_windowscan::IncludeAJson(std::string jsonfile) {
...
bool SusyMainAna_windowscan::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyMainAna_windowscan_cxx
