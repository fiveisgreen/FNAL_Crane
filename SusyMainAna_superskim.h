 ...
 */
	...
	// $Id: SusyMainAna_superskim.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
	//

#ifndef SusyMainAna_superskim_h
#define SusyMainAna_superskim_h
...
class SusyMainAna_superskim {
	...
	SusyMainAna_superskim(TTree *tree=0);
	virtual ~SusyMainAna_superskim();
...
#ifdef SusyMainAna_superskim_cxx
SusyMainAna_superskim::SusyMainAna_superskim(TTree *tree)
{
	if (tree == 0) {
...
SusyMainAna_superskim::~SusyMainAna_superskim()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyMainAna_superskim::GetEntry(Long64_t entry)
{
		// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyMainAna_superskim::LoadTree(Long64_t entry)
{
...
void SusyMainAna_superskim::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyMainAna_superskim::Initialize() {
...
void SusyMainAna_superskim::IncludeAJson(std::string jsonfile) {
...
bool SusyMainAna_superskim::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyMainAna_superskim_cxx
