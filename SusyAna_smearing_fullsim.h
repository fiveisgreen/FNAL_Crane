	...
	// Class:      SusyAna_smearing_fullsim.h
	// 
/*
 ...
 */
	...
	// $Id: SusyAna_smearing_fullsim.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
	//

#ifndef SusyAna_smearing_fullsim_h
#define SusyAna_smearing_fullsim_h
...
class SusyAna_smearing_fullsim {
	public :
	...
	SusyAna_smearing_fullsim(TTree *tree=0);
	virtual ~SusyAna_smearing_fullsim();
...
#ifdef SusyAna_smearing_fullsim_cxx
SusyAna_smearing_fullsim::SusyAna_smearing_fullsim(TTree *tree)
{
	if (tree == 0) {
...
SusyAna_smearing_fullsim::~SusyAna_smearing_fullsim()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyAna_smearing_fullsim::GetEntry(Long64_t entry)
{
		// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyAna_smearing_fullsim::LoadTree(Long64_t entry)
{
...
void SusyAna_smearing_fullsim::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyAna_smearing_fullsim::Initialize() {
...
void SusyAna_smearing_fullsim::IncludeAJson(std::string jsonfile) {
...
bool SusyAna_smearing_fullsim::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyAna_smearing_fullsim_cxx
