...
// $Id: SusyMainAna_MC_test.h,v 1.6 2012/08/31 11:33:53 bfrancis Exp $
//

#ifndef SusyMainAna_MC_test_h
#define SusyMainAna_MC_test_h
...
class SusyMainAna_MC_test {
	public :
		...
		SusyMainAna_MC_test(TTree *tree=0);
		virtual ~SusyMainAna_MC_test();
...
#ifdef SusyMainAna_MC_test_cxx
SusyMainAna_MC_test::SusyMainAna_MC_test(TTree *tree)
{
	if (tree == 0) {
...
SusyMainAna_MC_test::~SusyMainAna_MC_test()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyMainAna_MC_test::GetEntry(Long64_t entry)
{
	// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyMainAna_MC_test::LoadTree(Long64_t entry)
{
...
void SusyMainAna_MC_test::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyMainAna_MC_test::Initialize() {
...
void SusyMainAna_MC_test::IncludeAJson(std::string jsonfile) {
	...
	bool SusyMainAna_MC_test::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyMainAna_MC_test_cxx
