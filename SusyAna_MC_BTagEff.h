...
// $Id: SusyAna_MC_BTagEff.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
//

#ifndef SusyAna_MC_BTagEff_h
#define SusyAna_MC_BTagEff_h
...
class SusyAna_MC_BTagEff {
	public :
		...
		SusyAna_MC_BTagEff(TTree *tree=0);
		virtual ~SusyAna_MC_BTagEff();
...
#ifdef SusyAna_MC_BTagEff_cxx
SusyAna_MC_BTagEff::SusyAna_MC_BTagEff(TTree *tree)
{
	if (tree == 0) {
...
SusyAna_MC_BTagEff::~SusyAna_MC_BTagEff()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyAna_MC_BTagEff::GetEntry(Long64_t entry)
{
	// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyAna_MC_BTagEff::LoadTree(Long64_t entry)
{
...
void SusyAna_MC_BTagEff::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyAna_MC_BTagEff::Initialize() {
...
void SusyAna_MC_BTagEff::IncludeAJson(std::string jsonfile) {
	...
	bool SusyAna_MC_BTagEff::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyAna_MC_BTagEff_cxx
