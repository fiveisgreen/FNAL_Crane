	...
	// Class:      SusyAna_firstskim_Loose.cc
	// 
/*
 ...
 */
	...
	// $Id: SusyAna_firstskim_Loose.cc,v 1.12 2012/05/03 19:58:51 dwjang Exp $
	//

#define SusyAna_firstskim_Loose_cxx
...
#include "SusyAna_firstskim_Loose.h"
...
void SusyAna_firstskim_Loose::InitializePerEvent() {
	
}


bool SusyAna_firstskim_Loose::isSameObject(TLorentzVector& p1, TLorentzVector& p2) {
...
float SusyAna_firstskim_Loose::d0correction(TVector3& beamSpot, susy::Track& track) const {
...
bool SusyAna_firstskim_Loose::PassTrigger(TString path) {
	bool pass = false;
...
bool SusyAna_firstskim_Loose::PassTriggers() {
	bool pass = false;
...
void SusyAna_firstskim_Loose::Loop() {
	...
	for (Long64_t jentry=0; jentry < processNEvents; jentry++) {
		...
		if(phoMap != event->photons.end()) {
			for(std::vector<susy::Photon>::iterator it = phoMap->second.begin(); it != phoMap->second.end(); it++) {
				...
				if(is_skimmerPho_2012( it->momentum.Et(),
							it->caloPosition.Eta(),
							it->chargedHadronIso,
							it->neutralHadronIso,
							it->photonIso,
							...
							it->sigmaIetaIeta,
							event->rho25)) vvloose_photons.push_back(&*it);
		...
		if(vvloose_photons[0]->momentum.Et()<(phoEtThresh0-5) || vvloose_photons[1]->momentum.Et() < (phoEtThresh1-5)) continue;
		...
		while(i_p1 != vvloose_photons.end()-1 && !ok_photons){
			if((*i_p1)->momentum.Et()<phoEtThresh0-5) break;
			i_p2 = i_p1+1;
			while(i_p2 != vvloose_photons.end() && !ok_photons){
