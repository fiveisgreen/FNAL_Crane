...
class BTagWeight
{
	public:
		struct JetInfo {
			...
			JetInfo(float mceff,float datasf,float t=0.) : eff(mceff), sf(datasf) {}
			float eff; // probability of passing b-tag. may be eff or mistag rate.
			float sf; //   (eff in MC)/(eff in Data)
			//int tag;//this is not used
		};

		BTagWeight(int nTaggers_, int nJetRequirement_, std::vector<int> bTagRequirement_) : nTaggers(nTaggers_), nJetRequirement(nJetRequirement_) {
			for(int kTag = 0; kTag<nTaggers_;kTag++){
				 bTagRequirement.push_back(bTagRequirement_[kTag]);
				 bReqType.push_back(0);
			}
		}
		BTagWeight(int nTaggers_, int nJetRequirement_, std::vector<int> bTagRequirement_,std::vector<short> bReqType_ ): nTaggers(nTaggers_), nJetRequirement(nJetRequirement_) {
			for(int kTag = 0; kTag<nTaggers_;kTag++){
				 bTagRequirement.push_back(bTagRequirement_[kTag]);
...
float BTagWeight::weight(vector<vector<JetInfo> >jets){
	...
	if( njets < nJetRequirement || njets<=0) return 1.;
