...
// Class:      SusyAna_MC_BTagEff_arg.cc
//
/*
...
// $Id: SusyAna_MC_BTagEff_arg.cc,v 1.12 2012/05/03 19:58:51 dwjang Exp $
//

#define SusyAna_MC_BTagEff_arg_cxx
...
#include "SusyAna_MC_BTagEff_arg.h"
...
void SusyAna_MC_BTagEff_arg::InitializePerEvent() {

}


bool SusyAna_MC_BTagEff_arg::isSameObject(TLorentzVector& p1, TLorentzVector& p2) {
...
float SusyAna_MC_BTagEff_arg::d0correction(TVector3& beamSpot, susy::Track& track) const {
	return track.d0() - beamSpot.X()*std::sin(track.phi()) + beamSpot.Y()*std::cos(track.phi());
}

//float SusyAna_MC_BTagEff_arg::d0correction_(TVector3& beamSpot, susy::Track& track){
...
bool SusyAna_MC_BTagEff_arg::PassTrigger(TString path) {
...
bool SusyAna_MC_BTagEff_arg::isfarFromPhotons(TLorentzVector& lep, std::vector<susy::Photon*> & photons){
	bool isFar = true;
	for(std::vector<susy::Photon*>::iterator it = photons.begin(); it != photons.end(); it++) {
		...
		isFar &= ! SusyAna_MC_BTagEff_arg::isSameObject(lep,ltmp);
	}
	return isFar;
}


bool SusyAna_MC_BTagEff_arg::PassTriggers() {
	bool pass = false;
...
void remove_duplicate_photons(std::vector<susy::Photon*> & photons,bool keep_hardest_matched_photon=true){
	...
	void SusyAna_MC_BTagEff_arg::Loop(string which_MC_to_use) {

		cout<<"hello world"<<endl;
		ofstream firealarm;
		firealarm.open("tmpfirealarm.txt");
		firealarm << "SusyAna_MC_BTagEff_arg is on FIRE"<<endl;
		firealarm.close();
		...
		cout << "try making MCpoint"<<endl;
		MCpoint* thisMCpoint = setupMCpoint(which_MC_to_use);
		cout << "made MCpoint"<<endl;
...
//	if(!useTrigger){
	...
	//}
		...
		bool SusyAna_MC_BTagEff_arg::ok_muon(std::vector<susy::Muon>::iterator it){ //Yu Feng
		...
		bool SusyAna_MC_BTagEff_arg::ok_muon_AN_11_409(std::vector<susy::Muon>::iterator it,susy::Track& combinedTrack, float pVtx_Z){ //probably inner track instead
		...
		//use this one!
		bool SusyAna_MC_BTagEff_arg::ok_muon_DMoris(std::vector<susy::Muon>::iterator it_Mu,susy::Track& innerTrack){
		...
		//use this one!
		bool SusyAna_MC_BTagEff_arg::ok_ele(std::vector<susy::Electron>::iterator it_Ele){	//Dave Moris
		...
		bool SusyAna_MC_BTagEff_arg::ok_ele_AN_11_409(std::vector<susy::Electron>::iterator it_Ele, susy::Track& innerTrack, float pVtx_Z){
			//if(!it_Ele->isPF() and !it_Ele->passingMvaPreselection()) return false;
			float pt=it_Ele->momentum.Pt();
...
float SusyAna_MC_BTagEff_arg::JEC_MET(susy::MET* pfmet,std::vector<susy::PFJet*> ra3_pfjets,float &corrX, float &corrY, bool Debug_noisily){
...
float SusyAna_MC_BTagEff_arg::PhoEC_MET(float oldcorrmet,std::vector<susy::Photon*> photons, float &corrX, float &corrY, bool Debug_noisily){
		...
		bool SusyAna_MC_BTagEff_arg::ok_vtx(std::vector<susy::Vertex>::iterator it){
