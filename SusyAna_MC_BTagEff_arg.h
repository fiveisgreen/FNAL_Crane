...
// Class:      SusyAna_MC_BTagEff_arg.h
// 
/*
...
// $Id: SusyAna_MC_BTagEff_arg.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
//

#ifndef SusyAna_MC_BTagEff_arg_h
#define SusyAna_MC_BTagEff_arg_h
...
class SusyAna_MC_BTagEff_arg {
	public :
		...
		SusyAna_MC_BTagEff_arg(TTree *tree=0);
		virtual ~SusyAna_MC_BTagEff_arg();
		virtual Int_t    GetEntry(Long64_t entry);
		virtual Long64_t LoadTree(Long64_t entry);
		virtual void     Init(TTree *tree);
		virtual void     Loop(string which_MC_to_use);                          // event loop for main analysis
	...
	bool isfarFromPhotons(TLorentzVector& lep, std::vector<susy::Photon*> & photons);
		...
		//void MakeHistSet(TH1F** h, TString prefix, int nbins, float lower, float upper);
...
#ifdef SusyAna_MC_BTagEff_arg_cxx
SusyAna_MC_BTagEff_arg::SusyAna_MC_BTagEff_arg(TTree *tree)
{
	if (tree == 0) {
...
SusyAna_MC_BTagEff_arg::~SusyAna_MC_BTagEff_arg()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyAna_MC_BTagEff_arg::GetEntry(Long64_t entry)
{
	// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyAna_MC_BTagEff_arg::LoadTree(Long64_t entry)
{
...
void SusyAna_MC_BTagEff_arg::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyAna_MC_BTagEff_arg::Initialize() {
...
void SusyAna_MC_BTagEff_arg::IncludeAJson(std::string jsonfile) {
	...
	bool SusyAna_MC_BTagEff_arg::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyAna_MC_BTagEff_arg_cxx
