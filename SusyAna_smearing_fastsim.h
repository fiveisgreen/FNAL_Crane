	...
	// Class:      SusyAna_smearing_fastsim.h
	// 
/*
 ...
 */
	...
	// $Id: SusyAna_smearing_fastsim.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
	//

#ifndef SusyAna_smearing_fastsim_h
#define SusyAna_smearing_fastsim_h
...
class SusyAna_smearing_fastsim {
	public :
	...
	SusyAna_smearing_fastsim(TTree *tree=0);
	virtual ~SusyAna_smearing_fastsim();
...
#ifdef SusyAna_smearing_fastsim_cxx
SusyAna_smearing_fastsim::SusyAna_smearing_fastsim(TTree *tree)
{
	if (tree == 0) {
...
SusyAna_smearing_fastsim::~SusyAna_smearing_fastsim()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyAna_smearing_fastsim::GetEntry(Long64_t entry)
{
		// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyAna_smearing_fastsim::LoadTree(Long64_t entry)
{
...
void SusyAna_smearing_fastsim::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyAna_smearing_fastsim::Initialize() {
...
void SusyAna_smearing_fastsim::IncludeAJson(std::string jsonfile) {
...
bool SusyAna_smearing_fastsim::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyAna_smearing_fastsim_cxx
