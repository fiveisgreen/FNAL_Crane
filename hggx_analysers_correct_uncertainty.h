...
//			printf("lsb bin %i something is zero bin content %f sideband %f integral %f\n",ibin,h[0]->GetBinContent(ibin),lsb_int,B_integral);
	...
	for (int ibin = 0; ibin <= h[3]->GetNbinsX()+1; ibin++) {

		//if anything is going to make this calculation blow up in a divide by 0 error, don't do it. 
	       if(h[2]->GetBinContent(ibin) > 0 && usb_int >0  && h[0]->GetBinContent(ibin) > 0 && lsb_int >0 && B_integral>0 ){

		       float USB_minus_N = (float) usb_int - h[0]->GetBinContent(ibin);
		       float LSB_minus_N = (float) lsb_int - h[2]->GetBinContent(ibin);

		       float L = h[0]->GetBinError(ibin)/((float) lsb_int);
		       float U = h[2]->GetBinError(ibin)/((float) usb_int);

		       float sL2 =L*L*(pow((LSB_minus_N*h[0]->GetBinError(ibin)/(lsb_int*h[0]->GetBinContent(ibin))),2) +
				       (LSB_minus_N/(lsb_int*lsb_int)));
		       float sU2 =U*U*(pow((USB_minus_N*h[2]->GetBinError(ibin)/(usb_int*h[2]->GetBinContent(ibin))),2) +
				       (USB_minus_N/(usb_int*usb_int)));

		       h[3]->SetBinError(ibin,h[3]->GetBinContent(ibin)*sqrt( ((sU2+sL2)/pow(U+L,2)) + pow(B_integral_error/B_integral,2) ));
		       if(fabs(h[3]->GetBinContent(ibin) )< 0.0001 ) h[3]->SetBinError(ibin, 0.5*B_integral/(lsb_int+usb_int) ); //insert an uncertainty on zero = lsb has 1 and usb has 0. 
		}
