...
//enum cutmode {LEFT=0, RIGHT=1, DOUBLESIDED=2, HIGGS=3}; // moved to hggx_analysers.h
...
void format_plots_combined(){
	...
	string savewhat = "gpngC";//ggifpdfeps
//	string savewhat = "ggifpdfepsjpgpng";//ggifpdfeps
	...
	bool makeSigcompFullLinPlots = 1;
...
//    _____     __    __________  ___  __     __    
//   / ___/_ __/ /_  / __/ _/ _/ / _ \/ /__  / /____
//  / /__/ // / __/ / _// _/ _/ / ___/ / _ \/ __(_-<
//  \___/\_,_/\__/ /___/_//_/  /_/  /_/\___/\__/___/
//                                                  
//Cut Eff pltos 
//CutEff_topo_kinvar
	...
	if(makeCutEffPlots){
		for (int iTop = 0; iTop<nEventTopologies; iTop++) {
			for (int kKinVar = 0; kKinVar<nKinemVars; kKinVar++) {
				...
				enum cutmode thismode = KinVars[s_KinemVars[kKinVar]]->mode;

				string canvName = string("CutEff_")+s_KinemVars[kKinVar]+s_EventTopology_v2[iTop];
				...
				TPad *p2lin = new TPad("p2lin","",0.,0.145,1.,0.32);
				...
				//make arrays of the eff and eff comp hists
                                //TH1F* MC_hists[nDataAndMcFiles-1];
                                TH1F* Eff_hists[nDataAndMcFiles];//{bkg, mc1, mc2..}
                                TH1F* MC_EsOverRootEb[nDataAndMcFiles-1];//{Eff{mc1}/sqrt{Eff{bkg}}}

				if(printlevel>3) cout<<"attempt MakeCutEffScan on bkg"<<endl;
				Eff_hists[0] = MakeCutEffScan(h[3], thismode);
				if(printlevel>3) cout<<"Completed Eff_hits for bkg"<<endl;
                                int iter = 0;
                                for (int jFile = 0; jFile < nDataAndMcFiles; jFile++) { //for each file
                                        ...
                                        TH1F* MC_hist = KinVarHistMap[indexNames[jFile]][s_EventTopology[iTop]][s_KinemVars[kKinVar]][1];
					...
					if(printlevel>3) cout<<"Attempt Eff_hits for MC "<<jFile<<endl;
					Eff_hists[iter+1] = MakeCutEffScan(MC_hist, thismode);
					if(printlevel>3) cout<<"Completed Eff_hits"<<endl;
                                	MC_EsOverRootEb[iter] = CutEffComp(MC_hist, h[3], thismode);
					if(printlevel>3) cout<<"Completed CutEffComp for this file"<<endl;
                                        iter++;
                                }//end make hists

				p1lin->cd();
				PrettyHist(Eff_hists[0],dgBkgBoxColor);
				for (int i=0; i<nDataAndMcFiles-1; i++) { 
                                        PrettyHist(MC_EsOverRootEb[i],MCcolors[i],5,MClines[i]);
                                        PrettyHist(Eff_hists[i+1],    MCcolors[i],3,MClines[i]);
                                        PrettyMarker(MC_EsOverRootEb[i],MCcolors[i],kDot,0);
                                        PrettyMarker(Eff_hists[i+1],    MCcolors[i],kDot,0);
                                }
				MC_EsOverRootEb[0]->GetYaxis()->SetTitleOffset(0.8);
				MC_EsOverRootEb[0]->GetYaxis()->SetTitleSize(0.06);
				MC_EsOverRootEb[0]->GetYaxis()->SetLabelSize(0.06);
				MC_EsOverRootEb[0]->SetYTitle("Sig_Eff/sqrt(Bkg_Eff)");
				Eff_hists[0]->GetYaxis()->SetTitleOffset(0.5);
				Eff_hists[0]->GetXaxis()->SetTitleOffset(1.1);
				Eff_hists[0]->SetYTitle("Sig_Eff");

				for (int i=0; i<nDataAndMcFiles-2; i++) SameRange(MC_EsOverRootEb[0],MC_EsOverRootEb[i+1]);
				playNiceWithLegend(MC_EsOverRootEb[0],0.30,0.001);

				TLegend* l1 = new TLegend( 0.199749, 0.748552, 0.340452, 0.931423 ); 
				PrettyLegend(l1);
                                for (int i=0; i<nDataAndMcFiles-1; i++){
                                        string lablestring = string("MC Signal: ")+MCpoints[i+1][0]->s_DataAndMcFiles_v4;
                                        l1->AddEntry(MC_EsOverRootEb[i],lablestring.c_str());
                                }

				//I want to have the axes labeled nicely too. 


				MC_EsOverRootEb[0]->Draw();
				for (int i=1; i<nDataAndMcFiles-1; i++) MC_EsOverRootEb[i]->Draw("same");
                                l1->Draw("same");

				/////////////////////////////////////////////////////////
				p2lin->cd();
				Eff_hists[0]->GetYaxis()->SetRangeUser(-0.04,1.04);
                                TAxis * xx = Eff_hists[0]->GetXaxis();
                                TAxis * y = Eff_hists[0]->GetYaxis();
                                float fontsize = 0.25;
				...
				for (int i=0; i<nDataAndMcFiles-1; i++) Eff_hists[i+1]->Draw("samehist");
				Eff_hists[0]->Draw("samehist");
	...
	}//end if makeCutEffPlots


//     ___  __          ____     __     ___  __     __    
//    / _ )/ /_____ _  / __/_ __/ /    / _ \/ /__  / /____
//   / _  /  '_/ _ `/ _\ \/ // / _ \  / ___/ / _ \/ __(_-<
//  /____/_/\_\\_, / /___/\_,_/_.__/ /_/  /_/\___/\__/___/
//            /___/                                       
...
// ___|___|___|___|___|___|___|___|___|___|___|___|___|___|___|___|___|__
//            ___          _             _      __     ____
//           / _ \___ ___ (_)__ ____    | | /| / /__ _/ / /
//          / // / -_|_-</ / _ `/ _ \   | |/ |/ / _ `/ / / 
//         /____/\__/___/_/\_, /_//_/   |__/|__/\_,_/_/_/  
//                        /___/                            
// ___ ___ ___ ___ ___ ___ ___ ___ ___ ___ ___ ___ ___ ___ ___ ___ ___ __
// ___|___|___|___|___|___|___|___|___|___|___|___|___|___|___|___|___|__
// _|___|___|___|___|___|___|___|___|___|___|___|___|___|___|___|___|___|
...
//			gStyle->CreateGradientColorTable(NRGBs, stops, red, green, blue, NCont);
	...
	for (int iTop = 0; iTop<nEventTopologies; iTop++) {
		for (int kKinVar = 0; kKinVar<nKinemVars; kKinVar++) {
			...
			playNiceWithLegend(h[3],0.30,0.001);
			playNiceWithLegend(h[6],0.30,0.001);
			playNiceWithLegend(h[5],0.30,0.001);
				//			for (int i=0; i<nDataAndMcFiles-1; i++) SameRange(h[3],MC_hists[i]);
			for (int i=0; i<nDataAndMcFiles-1; i++){
				playNiceWithLegend(MC_hists[i],0.30,0.001);
	...
	if(showTag && (makeSigcompFullLinPlots || makeSigcompFullLogPlots)){
		for (int iTop = 0; iTop<nEventTopologies; iTop++) {
			for (int kKinVar = 0; kKinVar<nKinemVars; kKinVar++) {
				...
				playNiceWithLegend(h[3],0.30,0.001);
				playNiceWithLegend(h[1],0.30,0.001);
				playNiceWithLegend(h[5],0.30,0.001);
				playNiceWithLegend(h[6],0.30,0.001);
...
TH1F* CutEffComp(TH1F* sig, TH1F* bkg, cutmode approach){
		...
		//   "RIGHT": scan cut starting at right end
