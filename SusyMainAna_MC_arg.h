 ...
 */
	...
	// $Id: SusyMainAna_MC_arg.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
	//

#ifndef SusyMainAna_MC_arg_h
#define SusyMainAna_MC_arg_h
...
class SusyMainAna_MC_arg {
	...
	SusyMainAna_MC_arg(TTree *tree=0);
	virtual ~SusyMainAna_MC_arg();
...
#ifdef SusyMainAna_MC_arg_cxx
SusyMainAna_MC_arg::SusyMainAna_MC_arg(TTree *tree)
{
	if (tree == 0) {
...
SusyMainAna_MC_arg::~SusyMainAna_MC_arg()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusyMainAna_MC_arg::GetEntry(Long64_t entry)
{
		// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusyMainAna_MC_arg::LoadTree(Long64_t entry)
{
...
void SusyMainAna_MC_arg::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusyMainAna_MC_arg::Initialize() {
...
void SusyMainAna_MC_arg::IncludeAJson(std::string jsonfile) {
...
bool SusyMainAna_MC_arg::isInJson(Int_t run,Int_t lumi) {
...
#endif // #ifdef SusyMainAna_MC_arg_cxx
