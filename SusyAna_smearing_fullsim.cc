	// -*- C++ -*-
#define SusyAna_smearing_fullsim_cxx
...
#include "SusyAna_smearing_fullsim.h"
...
void SusyAna_smearing_fullsim::InitializePerEvent() {
	
}


bool SusyAna_smearing_fullsim::isSameObject(TLorentzVector& p1, TLorentzVector& p2) {
...
float SusyAna_smearing_fullsim::d0correction(TVector3& beamSpot, susy::Track& track) const {
	return track.d0() - beamSpot.X()*std::sin(track.phi()) + beamSpot.Y()*std::cos(track.phi());
}


bool SusyAna_smearing_fullsim::PassTrigger(TString path) {
...
bool SusyAna_smearing_fullsim::PassTriggers() {
	bool pass = false;
...
void SusyAna_smearing_fullsim::Loop() {
	ofstream firealarm;
	bool onfire = false;
	firealarm.open("tmpfirealarm.txt");
	firealarm << "SusyAna_smearing_fullsim is on FIRE"<<endl;
	firealarm.close();
///////////////////////////////////////////////////////
...
/*	if(enableFilter) {
	...
	ofstream eventlist;	
	if(makeEventsList) eventlist.open ("mcSmeraing_eventlist_fullsim.txt");
	ofstream tmvaOut;	
	...
	cout <<endl<< "Writing to Plots File mcSmeraing_fullsim.root"<<endl<<endl;
	TFile* fout = new TFile("mcSmeraing_fullsim.root","RECREATE");  
	fout->cd();
	ofstream logfile;
	logfile.open("mcSmeraing_fullsim.log");
...
//			if((useMVAphoP?loose_susy12_photons[0]->MVAcorrMomentum.Et():loose_susy12_photons[0]->momentum.Et()) < phoEtThresh0) cout<<"pho0 below threshold"<<endl;
		...
		TLorentzVector p1_smeared;
	        p0_smeared = ((*p_photonVector)[0]->MVAregEnergyAndErr.first * MCsmear_fullsim((*p_photonVector)[0]->caloPosition.Eta(), (*p_photonVector)[0]->r9 , rand_smearing ) / (*p_photonVector)[0]->momentum.E()) *(*p_photonVector)[0]->momentum;
                p1_smeared = ((*p_photonVector)[1]->MVAregEnergyAndErr.first * MCsmear_fullsim((*p_photonVector)[0]->caloPosition.Eta(), (*p_photonVector)[0]->r9 , rand_smearing ) / (*p_photonVector)[1]->momentum.E()) *(*p_photonVector)[1]->momentum;
		float mgg_smeared = (p0_smeared+p1_smeared).M();
		if(is_bar(p0_smeared.Eta()) && is_bar(p1_smeared.Eta() )) smeared_mgg->Fill(mgg_smeared);
...
bool SusyAna_smearing_fullsim::ok_muon(std::vector<susy::Muon>::iterator it){ //Yu Feng
...
bool SusyAna_smearing_fullsim::ok_muon_AN_11_409(std::vector<susy::Muon>::iterator it,susy::Track& combinedTrack, float pVtx_Z){ //probably inner track instead
...
//	std::map<TString,UChar_t>::iterator idPair = it->idPairs.find("muidGlobalMuonPromptTight");
	...
	//use this one!
bool SusyAna_smearing_fullsim::ok_muon_DMoris(std::vector<susy::Muon>::iterator it_Mu,susy::Track& innerTrack){
	...
	//use this one!
bool SusyAna_smearing_fullsim::ok_ele(std::vector<susy::Electron>::iterator it_Ele){	//Dave Moris
...
bool SusyAna_smearing_fullsim::ok_ele_AN_11_409(std::vector<susy::Electron>::iterator it_Ele, susy::Track& innerTrack, float pVtx_Z){
		//if(!it_Ele->isPF() and !it_Ele->passingMvaPreselection()) return false;
	float pt=it_Ele->momentum.Pt();
...
float SusyAna_smearing_fullsim::JEC_MET(susy::MET* pfmet,std::vector<susy::PFJet*> ra3_pfjets,float &corrX, float &corrY, bool Debug_noisily){ 
...
float SusyAna_smearing_fullsim::PhoEC_MET(float oldcorrmet,std::vector<susy::Photon*> photons, float &corrX, float &corrY, bool Debug_noisily){
...
bool SusyAna_smearing_fullsim::ok_vtx(std::vector<susy::Vertex>::iterator it){
...
void SusyAna_smearing_fullsim::MakeHistSet(TH1F** h, string prefix, int nbins, float lower, float upper){
...
void SusyAna_smearing_fullsim::MakeHistSet(TH1F** h, string prefix, int nbins, float*  bins){
	string names[nPhoMassAndBkgDists];
	string titles[nPhoMassAndBkgDists];
	string descriptions[nPhoMassAndBkgDists] = {"Lower Side Band",
		"in Higgs Tag Region","Upper Side Band","Background to Higgs Tag Region",
