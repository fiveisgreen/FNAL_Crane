...
//
//-------------------------------------------
float screanBtagMistag(int which, float beff);
...
void plotmean(TCanvas *yourC, int yourzone, TString tagger, TString TaggerStrength, TString DataPeriod)
{
	...
	for( int i = 2; i <= N; ++i )
	{
		...
		TString rangeEta_ = Form("SF(%1.1f #leq #eta %1.1f)",etamin[i],etamax[i]);
		leg->AddEntry(f[i],rangeEta_,"l");
	}
...
float screanBtagMistag(int which, float beff){
        //check that the b-tag mistag rate isn't 0, meaning that it wasn't filled in the histogram. 
        //return something sensible all the time. 
        //which indicates the CSV level 0 = L; 2 = T
        if(beff > 0.00001) return beff;
        else if(which == 0) return 0.3; //CSVL
        else if(which == 1) return 0.15; //CSVM
        else if(which == 2) return 0.05; //CSVT
        else return 1.0;
}//end screanBtagEff
