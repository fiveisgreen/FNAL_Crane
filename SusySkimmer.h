 ...
 */
	...
	// $Id: SusySkimmer.h,v 1.4 2011/06/08 16:28:40 dmason Exp $
	//

#ifndef SusySkimmer_h
#define SusySkimmer_h
...
#ifdef SusySkimmer_cxx
SusySkimmer::SusySkimmer(TTree *tree)
{
	if (tree == 0) {
...
SusySkimmer::~SusySkimmer()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t SusySkimmer::GetEntry(Long64_t entry)
{
		// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t SusySkimmer::LoadTree(Long64_t entry)
{
...
void SusySkimmer::Init(TTree *tree)
{
	if (!tree) return;
	fChain = tree;
	fCurrent = -1;
...
void SusySkimmer::IncludeAJson(std::string jsonfile) {
...
bool SusySkimmer::isInJson(Int_t run,Int_t lumi) {
