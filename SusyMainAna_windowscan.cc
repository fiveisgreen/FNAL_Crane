	...
	// -*- C++ -*-
#define SusyMainAna_windowscan_cxx
...
#include "SusyMainAna_windowscan.h"
...
void SusyMainAna_windowscan::InitializePerEvent() {
	
}


bool SusyMainAna_windowscan::isSameObject(TLorentzVector& p1, TLorentzVector& p2) {
...
float SusyMainAna_windowscan::d0correction(TVector3& beamSpot, susy::Track& track) const {
	
	return track.d0() - beamSpot.X()*std::sin(track.phi()) + beamSpot.Y()*std::cos(track.phi());
}


bool SusyMainAna_windowscan::PassTrigger(TString path) {
...
bool SusyMainAna_windowscan::PassTriggers() {
	bool pass = false;
...
void SusyMainAna_windowscan::Loop() {
	printLevel = 0;
...
//		std::vector<susy::Photon*>   tap_electrons;
	...
	//	that messes with the address of the start of the vector. 
		...
		if (loose_susy12_photons.size()<2 || 
			...
			(useMVAphoP?loose_susy12_photons[1]->MVAcorrMomentum.Et():loose_susy12_photons[1]->momentum.Et()) < phoEtThresh1) continue;
...
/*		topoCut["2JbML!m20Gbar2Mbb"] =    nJ >=2 && nbL ==2 && nbM >=1 && corrmet >20.0 && twoPhoBar && Mbb01_is_H;
		...
		//m_BTagWeight["3JbMLLm20Gbar2"] = new BTagWeight(3,3,bTagRequirement_MLL,bTypeExactLoose);
...
bool SusyMainAna_windowscan::ok_muon(std::vector<susy::Muon>::iterator it){ //Yu Feng
...
bool SusyMainAna_windowscan::ok_muon_AN_11_409(std::vector<susy::Muon>::iterator it,susy::Track& combinedTrack, float pVtx_Z){ //probably inner track instead
...
//	std::map<TString,UChar_t>::iterator idPair = it->idPairs.find("muidGlobalMuonPromptTight");
	...
	//use this one!
bool SusyMainAna_windowscan::ok_muon_DMoris(std::vector<susy::Muon>::iterator it_Mu,susy::Track& innerTrack){
	...
	//use this one!
bool SusyMainAna_windowscan::ok_ele(std::vector<susy::Electron>::iterator it_Ele){	//Dave Moris
...
bool SusyMainAna_windowscan::ok_ele_AN_11_409(std::vector<susy::Electron>::iterator it_Ele, susy::Track& innerTrack, float pVtx_Z){
		//if(!it_Ele->isPF() and !it_Ele->passingMvaPreselection()) return false;
	float pt=it_Ele->momentum.Pt();
...
float SusyMainAna_windowscan::JEC_MET(susy::MET* pfmet,std::vector<susy::PFJet*> ra3_pfjets,float &corrX, float &corrY, bool Debug_noisily){ 
...
float SusyMainAna_windowscan::PhoEC_MET(float oldcorrmet,std::vector<susy::Photon*> photons, float &corrX, float &corrY, bool Debug_noisily){
...
bool SusyMainAna_windowscan::ok_vtx(std::vector<susy::Vertex>::iterator it){
...
void SusyMainAna_windowscan::MakeHistSet(TH1F** h, string prefix, int nbins, float lower, float upper){
