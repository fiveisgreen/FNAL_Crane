...
void postanaana(){
	...
	for (int iTopo=0; iTopo<nEventTopologies; iTopo++) {
		...
		for (int i = 0; i<nPhoMassAndBkgDists; i++) tmpMapKinVar["ST"][i]->Rebin(5);
		for (int i = 0; i<nPhoMassAndBkgDists; i++) tmpMapKinVar["PtGG"][i]->Rebin(5);
		for (int i = 0; i<nPhoMassAndBkgDists; i++) tmpMapKinVar["HT"][i]->Rebin(5);
...
//	int nEventTopologies = 5; //keep same order as lsb_int: 
	...
	for (int iTopo=0; iTopo<nEventTopologies; iTopo++) {
		...
		fitmgg_simple(ha_diPhotonMass[iTopo], 105.,175.,&(g[iTopo]),&(mgg_fit_curve[iTopo]),chi2[iTopo], B_integral[iTopo], B_integral_error[iTopo],s_EventTopology_v2[iTopo]);
		//cout<<s_EventTopology[iTopo]<<" try TF1 "<<mgg_fit_curve[iTopo]->GetRandom()<<"  "<<mgg_fit_curve[iTopo]->GetName()<<endl;	
		printf("B_integral %s     %f +- %f  Chi2 %f\n",(char*)s_EventTopology[iTopo].c_str(), B_integral[iTopo],B_integral_error[iTopo],chi2[iTopo]);
	}
	if(debug>1) printf("ana finished get_bkg_integral\n");//integrals show up, but look obviously wrong, like the fits are horrific.
	
	for (int iTopo=0; iTopo<nEventTopologies; iTopo++) {
		for (int kKinVar = 0; kKinVar<nKinemVars; kKinVar++) {
			//printf("handing off B_integral %s     %f +- %f  Chi2 %f\n",(char*)s_EventTopology[iTopo].c_str(), B_integral[iTopo],B_integral_error[iTopo],chi2[iTopo]);
	...
	fout->cd();//no help there. 
	
	if(debug>3) printf("Wrote the set of mgg fit curves\n");
	for (int iTopo=0; iTopo < nEventTopologies; iTopo++){ 
	
		//cout<<"Try to writ TF1 for "<<s_EventTopology[iTopo]<<" try TF1 "<<mgg_fit_curve[iTopo]->GetRandom()<<endl;	
		mgg_fit_curve[iTopo]->Write(); 
		//cout<<"Wrote it "<<mgg_fit_curve[iTopo]->GetRandom()<< "  "<<mgg_fit_curve[iTopo]->GetName() <<endl;

	}//this write fails, seg fault.
	...
	for (int iTopo=0; iTopo<nEventTopologies; iTopo++){
		...
		TString TGtitle = "mgg"+s_EventTopology_v2[iTopo]+"TGraph";
		//char* title = (char*)(string("mgg")+s_EventTopology_v2[iTopo]+"TGraph").c_str();
		TCanvas *mggTGraph = new TCanvas(title.Data(),title.Data(),800, 800);
		//TCanvas *mggTGraph = newTCanvas(title,title);
		mggTGraph->cd();
		string graphTitle =string("Di-Photon mass spectra cuts ")+s_EventTopology_v4[iTopo]+";M^{#gamma #gamma} (GeV)";
		g[iTopo]->SetTitle((char*)graphTitle.c_str());
		g[iTopo]->Draw("AP");
		g[iTopo]->SetName(TGtitle.Data());
		g[iTopo]->Write();
