...
void SusyMainAna_Higgs::InitializePerEvent() {
	
}


bool SusyMainAna_Higgs::isSameObject(TLorentzVector& p1, TLorentzVector& p2) {
...
float SusyMainAna_Higgs::d0correction(TVector3& beamSpot, susy::Track& track) const {
	return track.d0() - beamSpot.X()*std::sin(track.phi()) + beamSpot.Y()*std::cos(track.phi());
}


bool SusyMainAna_Higgs::PassTrigger(TString path) {
...
bool SusyMainAna_Higgs::PassTriggers() {
	bool pass = false;
...
/*	nbins["EtaGG"] = 26;
	...
	if (!useTrigger) cout << endl << endl << "NOTE!! NOT USING THE TRIGGER!!!"<<endl<<endl<<endl;
...
bool SusyMainAna_Higgs::ok_muon(std::vector<susy::Muon>::iterator it){ //Yu Feng
...
bool SusyMainAna_Higgs::ok_muon_AN_11_409(std::vector<susy::Muon>::iterator it,susy::Track& combinedTrack, float pVtx_Z){ //probably inner track instead
...
//	std::map<TString,UChar_t>::iterator idPair = it->idPairs.find("muidGlobalMuonPromptTight");
	...
	//use this one!
bool SusyMainAna_Higgs::ok_muon_DMoris(std::vector<susy::Muon>::iterator it_Mu,susy::Track& innerTrack){
	...
	//use this one!
bool SusyMainAna_Higgs::ok_ele(std::vector<susy::Electron>::iterator it_Ele){	//Dave Moris
...
bool SusyMainAna_Higgs::ok_ele_AN_11_409(std::vector<susy::Electron>::iterator it_Ele, susy::Track& innerTrack, float pVtx_Z){
		//if(!it_Ele->isPF() and !it_Ele->passingMvaPreselection()) return false;
	float pt=it_Ele->momentum.Pt();
...
float SusyMainAna_Higgs::JEC_MET(susy::MET* pfmet,std::vector<susy::PFJet*> ra3_pfjets,float &corrX, float &corrY, bool Debug_noisily){ 
...
float SusyMainAna_Higgs::PhoEC_MET(float oldcorrmet,std::vector<susy::Photon*> photons, float &corrX, float &corrY, bool Debug_noisily){
...
bool SusyMainAna_Higgs::ok_vtx(std::vector<susy::Vertex>::iterator it){
...
void SusyMainAna_Higgs::MakeHistSet(TH1F** h, string prefix, int nbins, float lower, float upper){
